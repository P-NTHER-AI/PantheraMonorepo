...
router.get('/quote/buy/:agentAddress',
  [
    ...
    query('amount').isFloat({ min: 0.0001 }).withMessage('Amount must be at least 0.0001 CORE')
  ],
  validateRequest,
  async (req, res) => {
    try {
...
router.get('/balance/:agentAddress',
  [
    param('agentAddress').isEthereumAddress().withMessage('Invalid agent address'),
    query('userAddress').isEthereumAddress().withMessage('Invalid user address')
  ],
  validateRequest,
  async (req, res) => {
    try {
...
router.post('/buy',
  [
    ...
    body('agentAddress').isEthereumAddress().withMessage('Invalid agent address'),
    body('coreAmount').isFloat({ min: 0.001 }).withMessage('Core amount must be at least 0.001'),
  ...
  async (req, res) => {
    try {
      ...
      console.log(`🔥 Real buy order request: ${coreAmount} CORE for ${agentAddress} – tx: ${txHash}`);
...
router.post('/sell',
  ...
  async (req, res) => {
    try {
      ...
      if (result.success) {
        // For sell orders, we need to calculate the CORE amount from the result
...
router.post('/record-trade',
  [
    ...
    body('trader').isEthereumAddress().withMessage('Invalid trader address'),
    body('type').isIn(['buy', 'sell']).withMessage('Type must be buy or sell'),
    body('coreAmount').isString().withMessage('Core amount is required'),
