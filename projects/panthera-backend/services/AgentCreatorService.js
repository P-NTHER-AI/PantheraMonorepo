...
class AgentCreatorService {
  ...
  async directModelCall(session) {
    ...
    if (model.startsWith('llama3') || model.startsWith('mixtral')) {
      return await this.callGroqAPI(instruction, input, model);
    }

    // Together AI models
    else if (model.startsWith('deepseek') || model.startsWith('mistral') || model.startsWith('meta-llama')) {
      return await this.callTogetherAPI(instruction, input, model);
    }

    // Gemini models
    else if (model.startsWith('models/gemini')) {
      return await this.callGeminiAPI(instruction, input, model);
    }

    // Ollama models
    else if (model.startsWith('ollama-')) {
      return await this.callOllamaAPI(instruction, input, model.replace('ollama-', ''));
    }
