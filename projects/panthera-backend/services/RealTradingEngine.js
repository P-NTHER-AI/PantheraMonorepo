...
class RealTradingEngine {
  constructor(databaseService, websocketService) {
    ...
    this.config = {
      ...
      minTradeAmount: 0.001, // Minimum 0.001 CORE
...
async executeBuyOrder(userAddress, agentAddress, coreAmount, options = {}) {
  try {
    ...
    // Artık mock yok; ya signedTx ya txHash bekliyoruz
...
async executeSellOrder(userAddress, agentAddress, tokenAmount, options = {}) {
  ...
  async prepareBuyTransaction(userAddress, agentAddress, coreAmount, quote, options) {
    try {
      // This would prepare the actual blockchain transaction
      // For now, we'll return a mock transaction structure
      return {
        to: agentAddress,
        value: ethers.parseEther(coreAmount.toString()),
        data: '0x', // Would contain the actual contract call data
        from: userAddress,
        gasLimit: options.gasLimit || 200000,
        gasPrice: options.gasPrice || await this.provider.getFeeData().then(f => f.gasPrice)
      };
    } catch (error) {
...
// Execute transaction (no-mock: txHash veya signedTx bekler)
