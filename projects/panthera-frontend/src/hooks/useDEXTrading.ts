...
export const useDEXTrading = (_tokenAddress: string, _userAddress?: string) => {
  ...
  const getDEXQuote = useCallback(
    (_amount: string, _isBuy: boolean, _slippage: number = 2): Promise<DEXQuote> =>
      Promise.reject(new Error("DEX trading is not available on Algorand yet")),
    []
  );

  const executeDEXTrade = useCallback(
    (_quote: DEXQuote, _isBuy: boolean): Promise<string> =>
      Promise.reject(new Error("DEX trading is not available on Algorand yet")),
    []
  );
