...
import bondingCurveService, { AgentTokenInfo, BondingCurveError, BondingCurveQuote, BondingCurveTrade } from "../services/bondingCurve";
...
export const useBondingCurveTrading = (tokenAddress?: string) => {
  ...
  const fetchUserBalance = useCallback(async (tokenAddr: string, userAddr: string) => {
    ...
    try {
      const balance = await bondingCurveService.getTokenBalance(tokenAddr, userAddr);
      setState((prev) => ({ ...prev, userBalance: balance, balanceLoading: false }));
    } catch (error) {
  ...
  const getQuote = useCallback(
    async (type: "buy" | "sell", amount: string): Promise<TradingQuote | null> => {
      ...
      try {
        ...
        const quote: TradingQuote = {
          ...
          fees: {
            ...
            creatorFee: "0", // No creator fee in PANTHERA bonding curve
  ...
  const executeTrade = useCallback(
    async (type: "buy" | "sell", amount: string): Promise<boolean> => {
      ...
      try {
        ...
        if (result.success) {
          ...
          await Promise.all([fetchTokenInfo(tokenAddress), fetchUserBalance(tokenAddress, userAddress)]);

          // Emit custom event for UI updates
          window.dispatchEvent(
            new CustomEvent("tradingSuccess", {
              detail: {
                type,
