...
export const useChartData = (
  ...
  const generateCandlesFromTrades = useCallback(
    (trades: TradeData[], interval: string): CandleData[] => {
  ...
  const fetchHistoricalData = useCallback(async () => {
    ...
    try {
      ...
      if (tradesResponse.data?.success && Array.isArray(tradesResponse.data?.data) && tradesResponse.data.data.length > 0) {
        trades = tradesResponse.data.data.map((trade: TradeApiData) => {
          const ts = new Date(trade.timestamp).getTime();
          const price = Number(trade.price) || 0;
          const amount =
            trade.coreAmount !== undefined
      ...
      const marketPrice = candles.length > 0 ? candles[candles.length - 1].close : trades.length > 0 ? trades[0].price : null;

      let priceChange24h: number | null = null;
      if (candles.length >= 2) {
        const latest = candles[candles.length - 1];
        const previous = candles[0];
        priceChange24h = previous.close > 0 ? ((latest.close - previous.close) / previous.close) * 100 : 0;
      }
      ...
      setState((prev) => ({
        ...
        trades,
        loading: false,
        error: candles.length === 0 && trades.length === 0 ? "No trading history available yet. Start trading to see chart data." : null,
        lastUpdate: new Date(),
      ...
      if (candles.length === 0 && trades.length === 0) {
        ...
        setState((prev) => ({
          ...
          trades: [],
          loading: false,
          error: "No trading history available yet. Start trading to see chart data.",
          lastUpdate: new Date(),
        }));
      }
    } catch (error) {
  ...
  const updateWithNewTrade = useCallback(
    (trade: TradeData) => {
      setState((prev) => {
        ...
        const existingCandleIndex = newCandles.findIndex((candle) => candle.date.getTime() === candleTime);
  ...
  const handleRealTimeTradeUpdate = useCallback(
    (data: RealTimeTradeUpdate) => {
  ...
  useEffect(() => {
    ...
    handleConnectionStatusChange(websocketService.isConnected() ? "connected" : "disconnected");
