...
const fmtUSD = (n?: number) => (n == null ? "" : `$${Number(n).toLocaleString(undefined, { maximumFractionDigits: 2 })}`);
...
const Profile: React.FC<ProfileProps> = ({ onBack }) => {
  ...
  const [bio, setBio] = React.useState("AI enthusiast and DeFi researcher. Building the future of decentralized AI agents.");
  ...
  // Quick stats (derived)
  const agentsCreated = React.useMemo(() => (Array.isArray(userAgents) ? userAgents.length : 0), [userAgents]);

  const totalPortfolioValue = React.useMemo(
    () => (Array.isArray(portfolio) ? portfolio.reduce((sum: number, p: PortfolioItem) => sum + (Number(p.value) || 0), 0) : 0),
    [portfolio]
  );
      ...
      try {
        ...
        const rows: PortfolioItem[] = await Promise.all(
          userAgents.map(async (ag: any) => {
            ...
            const value = Number.isFinite(holdings * market.priceUsd) ? Number(holdings * market.priceUsd) : 0;
        ...
        try {
          ...
          for (const ev of raw) {
            items.push({
              ...
              icon: ev.icon ?? (ev.type === "earned" ? "💰" : ev.type === "created" ? "🤖" : ev.type === "milestone" ? "🎯" : "🛒"),
            });
          }
        } catch {
          /* yoksa fallback'lere geç */
        }
  ...
  }, [profile?.username, profile?.socialLinks?.twitter, profile?.socialLinks?.discord, profile?.socialLinks?.github]);
  ...
  function normalizeSocial(kind: "twitter" | "discord" | "github", value: string) {
    ...
    const base = kind === "twitter" ? "https://twitter.com/" : kind === "discord" ? "https://discord.gg/" : "https://github.com/";
    return base + v;
  }
        ...
        <div className="absolute inset-0 flex items-center justify-center pt-8">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-2 mb-2">
              <h1 className="text-white text-3xl font-bold">{profileLoading ? "..." : profile?.username ?? ""}</h1>
              {profile?.isVerified && (
            ...
            <div className="flex items-center justify-center gap-2 text-[#a0a0a0]">
              <Calendar size={14} />
              <span className="text-sm">{profileLoading ? "Joined —" : `Joined ${profile?.joinDate ?? "-"}`}</span>{" "}
            </div>
          </div>
        </div>
      ...
      <div className="px-8 py-6">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setIsEditingBio(!isEditingBio)} className="text-[#d8e9ea] hover:text-[#b8d4d6] transition-colors">
              <Edit size={16} />
            </button>
          </div>
          {isEditingBio ? (
          ...
          <div className="flex gap-4">
            <a
              ...
              className={`flex items-center gap-2 ${
                profile?.socialLinks?.twitter ? "text-[#a0a0a0] hover:text-[#d8e9ea]" : "text-[#555] cursor-not-allowed"
              }`}
            >
              <TwitterIcon size={18} />
            ...
            <a
              ...
              className={`flex items-center gap-2 ${
                profile?.socialLinks?.discord ? "text-[#a0a0a0] hover:text-[#d8e9ea]" : "text-[#555] cursor-not-allowed"
              }`}
            >
            ...
            <a
              ...
              rel="noreferrer"
              className={`flex items-center gap-2 ${
                profile?.socialLinks?.github ? "text-[#a0a0a0] hover:text-[#d8e9ea]" : "text-[#555] cursor-not-allowed"
              }`}
            >
              <GithubIcon size={18} />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4">
            ...
            <div className="text-2xl font-bold text-white">{agentsLoading ? "—" : agentsCreated}</div>
          </div>
          ...
          <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4">
            ...
            <div className="text-2xl font-bold text-white">
              {portfolioLoading ? "—" : `$${totalPortfolioValue.toLocaleString(undefined, { maximumFractionDigits: 2 })}`}
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 py-3 px-1 text-sm font-medium transition-colors ${
                    activeTab === tab.id ? "text-[#d8e9ea] border-b-2 border-[#d8e9ea]" : "text-[#a0a0a0] hover:text-white"
                  }`}
                >
                  <Icon size={16} />
                  {tab.label}
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 bg-gradient-to-br from-[#d8e9ea] to-[#b8d4d6] rounded-xl flex items-center justify-center text-xl overflow-hidden">
                            ...
                            <span className={`text-xl ${agent.image ? "hidden" : "flex"}`}>🤖</span>
                          </div>
                          <div>
                            <h4 className="text-white font-semibold">{agent.name}</h4>
                            <div
                              className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getStatusBg(
                                agent.status
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  <div className="text-white text-xl font-bold">
                    $
                    {(portfolio ?? [])
                      .reduce((sum: number, i: PortfolioItem) => sum + i.value, 0)
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {portfolioLoading ? (
                        <tr>
                          <td className="p-4 text-[#a0a0a0]" colSpan={5}>
                            Loading portfolio...
                          </td>
                        </tr>
                      ) : (portfolio ?? []).length === 0 ? (
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            {activity.amount != null && <span className="text-[#10b981] font-medium">{fmtUSD(activity.amount)}</span>}
                          </div>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-6">
                ...
                <div className="space-y-4">
                  ...
                  <button disabled className="bg-[#d8e9ea] text-black px-4 py-2 rounded-lg font-medium opacity-60 cursor-not-allowed">
                    Enable 2FA
                  </button>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative w-12 h-6 rounded-full transition-colors ${
                        settingsForm.emailNotifications ? "bg-[#d8e9ea]" : "bg-[#2a2a2a]"
                      }`}
                    >
                      <div
                        className={`absolute top-1 ${
                          settingsForm.emailNotifications ? "right-1" : "left-1"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative w-12 h-6 rounded-full transition-colors ${
                        settingsForm.pushNotifications ? "bg-[#d8e9ea]" : "bg-[#2a2a2a]"
                      }`}
                    >
                      <div
                        className={`absolute top-1 ${
                          settingsForm.pushNotifications ? "right-1" : "left-1"
                        } w-4 h-4 bg-white rounded-full shadow-md`}
                      ></div>
                    </button>
                  </div>
