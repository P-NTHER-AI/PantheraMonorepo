import { AlertTriangle, Clock, DollarSign, Info, Percent, TrendingDown, TrendingUp, Zap } from "lucide-react";
...
export const ProfessionalQuoteDisplay: React.FC<ProfessionalQuoteDisplayProps> = ({
    ...
    return (
      <div className={`bg-[#1a1a1a] border border-red-500/20 rounded-xl p-4 ${className}`}>
        ...
        <div className="mt-3 p-3 bg-red-500/10 rounded-lg">
          <p className="text-xs text-red-200">If this token is graduated, try using DEX trading instead of bonding curve.</p>
        </div>
      </div>
    );
  }
  ...
  const outputAmount = isBuy ? quote.tokensReceived : quote.coreReceived;
  const inputAmount = isBuy ? quote.coreAmount : quote.tokenAmount;

  const priceImpactColor = quote.priceImpact > 5 ? "text-red-400" : quote.priceImpact > 2 ? "text-yellow-400" : "text-green-400";

  const slippageColor = quote.slippage > 3 ? "text-red-400" : quote.slippage > 1 ? "text-yellow-400" : "text-green-400";
    ...
    <div className={`bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4 ${className}`}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-white">{isBuy ? "Buy" : "Sell"} Quote</span>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-[#2a2a2a] rounded-lg p-3">
            ...
            <div className={`text-sm font-medium ${priceImpactColor}`}>{quote.priceImpact.toFixed(2)}%</div>
          </div>
          ...
          <div className="bg-[#2a2a2a] rounded-lg p-3">
            ...
            <div className={`text-sm font-medium ${slippageColor}`}>{quote.slippage.toFixed(2)}%</div>
          </div>
        </div>
        ...
        <div
          className={`p-3 rounded-lg ${
            isGraduated ? "bg-yellow-500/10 border border-yellow-500/20" : "bg-blue-500/10 border border-blue-500/20"
          }`}
        >
          <div className="flex items-center gap-2 mb-1">
            {isGraduated ? <Zap className="w-4 h-4 text-yellow-400" /> : <TrendingUp className="w-4 h-4 text-blue-400" />}
        ...
        {quote.priceImpact > 5 && (
          <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
            ...
            <p className="text-xs text-red-200">This trade will significantly impact the token price. Consider reducing your trade size.</p>
          </div>
        )}
      </div>
    </div>
