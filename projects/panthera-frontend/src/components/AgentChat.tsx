...
const genUUID = (): string => {
  ...
  const bytes: Uint8Array = g ? g(new Uint8Array(16)) : new Uint8Array(Array.from({ length: 16 }, () => Math.floor(Math.random() * 256)));
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {messages.map((message) => (
            <div key={message.id} className={`flex ${message.type === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[80%] rounded-lg p-3 ${message.type === "user" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-100"}`}
              >
                <div className="flex items-start space-x-2">
                  {message.type === "agent" && <Bot className="w-4 h-4 mt-0.5 text-blue-400 flex-shrink-0" />}
                  {message.type === "user" && <User className="w-4 h-4 mt-0.5 text-white flex-shrink-0" />}
                  <div className="flex-1">
                    <p className="text-sm whitespace-pre-wrap">{message.message}</p>
                    {message.error && <p className="text-red-400 text-xs mt-1">{message.error}</p>}
                  </div>
                </div>
              </div>
            </div>
