import { Activity, ArrowDownRight, ArrowUpRight, CheckCircle, DollarSign, MessageCircle, Minus, Shield, Star, Users } from "lucide-react";
...
const AgentCard: React.FC<AgentCardProps> = ({ agent, onCardClick, onChatClick, onTradeClick }) => {
  ...
  const [displayPrice, setDisplayPrice] = useState(Number(agent.currentPrice) || 0);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 p-6">
        ...
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="relative flex-shrink-0">
            <div className="w-14 h-14 relative group-hover:scale-105 transition-transform duration-300">
              ...
              <div className="relative w-full h-full bg-gradient-to-br from-[#2a2a2a] to-[#1a1a1a] rounded-2xl border border-[#3a3a3a] flex items-center justify-center overflow-hidden">
                ...
                <span className={`text-xl ${agent.image ? "hidden" : "flex"}`}>{agent.avatar || "🤖"}</span>
              </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-white font-bold text-lg truncate">{agent.name}</h3>
              <span className="text-[#d8e9ea]/70 text-sm font-semibold bg-[#2a2a2a] px-2 py-0.5 rounded-lg">${agent.symbol}</span>
            </div>

            <p
              className="text-[#a0a0a0] text-sm leading-relaxed mb-2 overflow-hidden"
              style={{
                display: "-webkit-box",
                WebkitLineClamp: 2,
            ...
            <div className="flex items-center gap-3 text-xs text-[#a0a0a0]">
              ...
              {agent.category && (
                <>
                  <span>•</span>
                  <span className="text-[#d8e9ea]/80">{agent.category}</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="bg-[#2a2a2a]/20 rounded-xl p-4 mb-4 border border-[#3a3a3a]/30 backdrop-blur-sm">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <div
                className={`text-white font-bold text-xl transition-all duration-300 ${
                  priceChangeAnimation === "price-up"
                    ? "text-green-400 scale-110"
                    : priceChangeAnimation === "price-down"
          ...
          <div className="text-[#a0a0a0] text-sm">
            Market Cap: <span className="text-[#d8e9ea] font-semibold">{formatNumber(marketCap)}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="text-center">
            ...
            <div className="text-[#d8e9ea] font-bold text-sm">{formatNumber(volume24h)}</div>
            <div className="text-[#a0a0a0] text-xs">24h Volume</div>
          </div>
          ...
          <div className="text-center">
            ...
            <div className="text-[#d8e9ea] font-bold text-sm">{formatNumber(holders, { decimals: 0 })}</div>
            <div className="text-[#a0a0a0] text-xs">Holders</div>
          </div>
          ...
          <div className="text-center">
            ...
            <div className="text-[#d8e9ea] font-bold text-sm">{formatNumber(totalSupply, { compact: true })}</div>
            <div className="text-[#a0a0a0] text-xs">Total Supply</div>
          </div>
          ...
          <div className="text-center">
            ...
            <div className="text-[#d8e9ea] font-bold text-sm">{agent.chatCount || 0}</div>
            <div className="text-[#a0a0a0] text-xs">Chats</div>
          </div>
        </div>
