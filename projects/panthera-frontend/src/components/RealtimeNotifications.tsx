import { MessageCircle, TrendingDown, TrendingUp, X, Zap } from "lucide-react";
...
const RealtimeNotifications: React.FC<RealtimeNotificationsProps> = ({ notifications, onDismiss }) => {
  ...
  return (
    <div className="fixed top-4 right-4 z-50 space-y-2">
      ...
      {visibleNotifications.map((notification) => (
        <div
          key={notification.id}
          className={`flex items-start gap-3 p-4 rounded-lg border backdrop-blur-sm transition-all duration-300 hover:scale-105 ${getNotificationColor(
            notification.type
          )}`}
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-white font-medium leading-tight">{getNotificationMessage(notification)}</p>
            <p className="text-xs text-[#a0a0a0] mt-1">{formatTime(notification.timestamp)}</p>
          </div>

          <button onClick={() => onDismiss(notification.id)} className="flex-shrink-0 text-[#a0a0a0] hover:text-white transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
      ))}
