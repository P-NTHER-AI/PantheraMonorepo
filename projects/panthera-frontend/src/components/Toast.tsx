...
export const Toast: React.FC<ToastProps> = ({ toasts, onDismiss }) => {
  return (
    <div className="fixed top-4 right-4 z-50 space-y-3">
      {toasts.map((t) => (
        <div key={t.id} className={`min-w-[260px] max-w-sm border rounded-lg p-3 shadow-lg ${typeStyles[t.type || "info"]}`}>
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            <button onClick={() => onDismiss(t.id)} className="text-xs underline opacity-70 hover:opacity-100">
              Dismiss
            </button>
          </div>
        </div>
