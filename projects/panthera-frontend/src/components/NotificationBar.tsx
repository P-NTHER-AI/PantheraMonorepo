  ...
  useEffect(() => {
    if (wsNotifications && wsNotifications.length > 0) {
      const newNotifications = wsNotifications.slice(0, 10).map((event, index) => ({
        ...
        type: event.type === "agentCreated" ? ("create" as const) : event.type === "tokensPurchased" ? ("buy" as const) : ("sell" as const),
        user: event.data?.creator?.slice(0, 6) + "..." || "Unknown",
        amount: event.data?.coreAmount ? parseFloat(event.data.coreAmount) : undefined,
        ...
        return (
          <span className="whitespace-nowrap">
            <span className="text-black px-2 py-1 rounded mr-2" style={{ backgroundColor: "#d8e9ea" }}>
              🤖 {notification.user} bought {notification.amount} SOL of {notification.agent} 🟡 | mcap: $
              {(notification.marketCap! / 1000).toFixed(1)}K
            </span>
          </span>
        );
      case "create":
