import { Activity, AlertCircle, CheckCircle, Clock, DollarSign, Shield, TrendingUp, Users, Zap } from "lucide-react";
          ...
          if (tradesResponse.ok) {
            ...
            if (Array.isArray(trades) && trades.length > 0) {
              ...
              const realEvents: TradingEvent[] = trades.map((trade: any) => ({
                ...
                amount: trade.coreAmount || trade.amount || "0",
                txHash: trade.txHash,
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          {tradingEvents.length > 0 && (
            <button onClick={() => setTradingEvents([])} className="text-xs text-gray-400 hover:text-white transition-colors">
              Clear
            </button>
          )}
        </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <span
                      className={`text-sm font-medium ${
                        event.type === "success" ? "text-green-400" : event.type === "error" ? "text-red-400" : "text-yellow-400"
                      }`}
                    >
                      {event.tradeType ? (
                        <span
                          className={`px-2 py-1 rounded text-xs font-bold ${
                            event.tradeType === "buy" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"
                          }`}
                        >
                    ...
                    <span className="text-xs text-gray-400 uppercase">{event.tradingMethod.replace("-", " ")}</span>
                  </div>
