import { DollarSign, Target, TrendingUp, Trophy, Users, Zap } from "lucide-react";
...
export const BondingCurveProgress: React.FC<BondingCurveProgressProps> = ({
    ...
    return (
      <div className={`${className}`}>
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-xs text-[#d8e9ea] font-medium">{formatProgress(progressPercentage)}%</span>
        </div>
  ...
  if (variant === "compact") {
    return (
      <div className={`bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-3 ${className}`}>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            {statusInfo.icon}
            <span className={`text-sm font-medium ${statusInfo.color}`}>{statusInfo.text}</span>
          </div>
          <span className="text-sm text-white font-medium">{formatProgress(progressPercentage)}%</span>
        </div>
  ...
  return (
    <div className={`bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4 ${className}`}>
        ...
        {!isGraduated && (
          ...
          >
            ...
            <div className="absolute -top-6 left-1/2 transform -translate-x-1/2">
              <div className="text-xs text-yellow-400 font-medium whitespace-nowrap">Graduation</div>
            </div>
          </div>
        )}
      ...
      {/* Status Text */}
      <div className="flex items-center justify-between mb-3">
        <div className="text-sm text-gray-400">{statusInfo.subtext}</div>
        ...
        <div className="grid grid-cols-3 gap-3 pt-3 border-t border-[#2a2a2a]">
          {currentPrice > 0 && (
            <div className="text-center">
              ...
              <div className="text-[#d8e9ea] font-bold text-xs">{currentPrice.toFixed(12)}</div>
              <div className="text-[#a0a0a0] text-xs">Price</div>
            </div>
          )}
          ...
          {marketCap > 0 && (
            <div className="text-center">
              ...
              <div className="text-[#d8e9ea] font-bold text-xs">{formatNumber(marketCap, { compact: true })}</div>
              <div className="text-[#a0a0a0] text-xs">Market Cap</div>
            </div>
          )}
          ...
          {holders > 0 && (
            <div className="text-center">
              ...
              <div className="text-[#d8e9ea] font-bold text-xs">{formatNumber(holders, { decimals: 0 })}</div>
      ...
      {!isGraduated && progressPercentage >= 80 && (
        <div className={`mt-3 p-3 ${statusInfo.bgColor} rounded-lg`}>
          <div className={`text-sm font-medium ${statusInfo.color} mb-1`}>🎉 Graduation Benefits</div>
      ...
      {isGraduated && (
        <div className={`mt-3 p-3 ${statusInfo.bgColor} rounded-lg`}>
          <div className={`text-sm font-medium ${statusInfo.color} mb-1`}>🏆 Token Graduated Successfully!</div>
          <div className="text-xs text-gray-300 space-y-1">
