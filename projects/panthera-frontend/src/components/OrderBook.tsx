...
const OrderBook: React.FC<OrderBookProps> = ({ agentAddress, currentPrice = 0 }) => {
  ...
  useEffect(() => {
    if (latestTrade) {
      ...
      const coreAmount = parseFloat(latestTrade.coreAmount || "0");
      ...
      // Update 24h volume
      setVolume24h((prev) => prev + coreAmount);
    ...
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between p-4 border-b border-[#2a2a2a]">
        <div className="flex items-center space-x-2">
          ...
          {error && <span className="text-xs text-red-400 bg-red-500/10 px-2 py-1 rounded">Error: {error}</span>}
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          {/* Last Update */}
          {lastUpdateTime && <div className="text-xs text-[#a0a0a0]">Updated: {lastUpdateTime.toLocaleTimeString()}</div>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 p-4">
        ...
        <div className="space-y-4">
          ...
          {/* Recent Trades */}
          <div className="space-y-2">
            <div className="text-sm font-medium text-[#a0a0a0] border-b border-[#2a2a2a] pb-2">Recent Trades</div>
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">
              {recentTrades.slice(0, 10).map((trade, index) => (
                ...
                >
                  <span className={trade.type === "buy" ? "text-green-400" : "text-red-400"}>{formatPrice(trade.price)}</span>
                  <span className="text-[#a0a0a0]">{formatAmount(trade.amount)}</span>
                  <span className="text-[#666]">{formatTime(trade.timestamp)}</span>
                </div>
              ))}

              {recentTrades.length === 0 && <div className="text-center text-[#666] py-4">No recent trades</div>}
            </div>
          </div>
        </div>
