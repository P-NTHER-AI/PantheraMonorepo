import { ArrowLeft, ExternalLink, RefreshCw, TrendingUp, Wallet } from "lucide-react";
...
const Portfolio: React.FC<PortfolioProps> = ({ onBack }) => {
  ...
  const fetchPortfolioData = useCallback(async () => {
    ...
    try {
      ...
      const processedHoldings: TokenHolding[] = mockHoldings.map((holding) => {
      ...
      const bestPerformer =
        processedHoldings.length > 0
          ? processedHoldings.reduce((best, current) => (current.unrealizedPnLPercentage > best.unrealizedPnLPercentage ? current : best))
          : null;
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-gradient-to-br from-[#d8e9ea] to-[#b8d4d6] rounded-xl flex items-center justify-center">
                      <span className="text-black font-bold text-lg">{holding.symbol.charAt(0)}</span>
                    </div>
                    <div>
                  ...
                  <div className="text-right">
                    <p className="text-white font-semibold">
                      {formatNumber(parseFloat(holding.balance))} {holding.symbol}
                    </p>
