...
export const TradingViewChart: React.FC<TradingViewChartProps> = ({ agentAddress, tokenSymbol, className = "" }) => {
  ...
  const { candles, trades, loading, error, livePrice, lastUpdate, priceChange24h, connectionStatus } = useChartData(agentAddress, {
  ...
  useEffect(() => {
    ...
    if (dedupedCandles.length !== candlestickData.length) {
      console.log("⚠️ Duplicate candle times trimmed:", candlestickData.length, "→", dedupedCandles.length);
    }
            ...
            <div className="flex items-center space-x-4">
              <div className="text-right">
                ...
                <div className={`text-sm ${priceChange >= 0 ? "text-green-400" : "text-red-400"}`}>
                  {priceChange >= 0 ? "+" : ""}
                  {formatNumber(priceChange.toString(), { maxDecimals: 10, minDecimals: 6 })}({priceChangePercent >= 0 ? "+" : ""}
      ...
      <div className="p-3 border-t border-[#2a2a2a] text-xs text-gray-400">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            {trades && trades.length > 0 && <div>Trades: {trades.length} analyzed</div>}
          ...
          <div className="flex items-center space-x-2">
            <span>PANTHERA Professional Chart</span>
            <span>•</span>
